...
class EglCore {
    ...
    constructor(sharedContext: EGLContext?, flags: Int) {
        ...
        FLogger.d(TAG, "EGLContext created, client version " + values[0])
    }
    ...
    private fun getConfig(flags: Int, version: Int): EGLConfig? {
        ...
        if (!EGL14.eglChooseConfig(
            ...
            FLogger.w(TAG, "unable to find RGB8888 / $version EGLConfig")
            return null
        }
        return configs[0]
    }
...
//    fun createWindowSurface(surface: SurfaceHolder): EGLSurface = createWindowSurface(surface)
    ...
    fun makeCurrent(eglSurface: EGLSurface?) {
        if (mEGLDisplay === EGL14.EGL_NO_DISPLAY) {
            ...
            FLogger.d(TAG, "NOTE: makeCurrent w/o display")
        }
